...
int dso_bulk_conversation(libusb_device_handle *dso_device_handle,
							unsigned char ep,
							unsigned char *data,
							int length, int tryCount);

// Makes blocking control transfer
int libusbdso_control_transfer(libusb_device_handle *dev_handle,
...
// Makes blocking bulk transfer
int libusbdso_bulk_transfer(libusb_device_handle *dev_handle,
							unsigned char endpoint,
							unsigned char *buffer,
							int length,
							int *actual_length);

int add_known_device(struct t_dso_known_device *known_device) {
...
struct t_dso_known_device* is_device_known(uint16_t vendor_id,
											uint16_t product_id) {
...
void* libusbdso_event_thread_func(void *ctx) {
...
int LIBUSB_CALL libusbdso_hotplug_callback(libusb_context *ctx,
											libusb_device *dev,
...
int libusbdso_start(struct t_dso_driver_descriptor *driver_desc) {
...
int libusbdso_open_device(void *dso_device_handle) {
...
int libusbdso_write_device(void *dso_device_handle, unsigned char *data,
		int length) {
	struct t_dso_device_descriptor *dso_desc;
	int status;
...
int libusbdso_read_device(void *dso_device_handle, unsigned char *data, int length) {
	struct t_dso_device_descriptor *dso_desc;
	int status;
...
int libusbdso_control_in_device(void *dso_device_handle,
								uint8_t request,
								uint16_t value,
								uint16_t index,
								unsigned char *data,
	...
	if (dso_device_handle) {
		...
		dso_desc = dso_device_handle;
		if (dso_desc->dso_device_handle) {
			libusb_status_code = libusbdso_control_transfer(dso_desc->dso_device_handle,
...
int libusbdso_control_out_device(void *dso_device_handle,
									uint8_t request,
									uint16_t value,
									uint16_t index,
									unsigned char *data,
...
int libusbdso_close_device(void *dso_device_handle) {
	struct t_dso_device_descriptor *dso_desc;
	int status;
...
const char* libusbdso_get_device_name(void *dso_device_handle) {
	if (dso_device_handle) {
...
uint8_t libusbdso_get_device_index(void *dso_device_handle) {
	if (dso_device_handle) {
...
void* libusbdso_get_win_device_handle(void *dso_device_handle) {
	if (dso_device_handle) {
...
void libusbdso_set_win_device_handle(void *dso_device_handle, void *handle) {
	if (dso_device_handle) {
		((struct t_dso_device_descriptor*) dso_device_handle)->win_device_handle =
				handle;
	}
}

uint16_t libusbdso_get_vendor_id(void *dso_device_handle) {
	if (dso_device_handle) {
...
uint16_t libusbdso_get_product_id(void *dso_device_handle) {
	if (dso_device_handle) {
...
const char* libusbdso_last_error() {
	return libusb_error_name(libusb_status_code);
}

int libusbdso_last_error_code() {
...
int dso_bulk_conversation(libusb_device_handle *dso_device_handle,
	...
	while (length && (try_counts_done < try_counts)) {
		libusb_status_code = libusbdso_bulk_transfer(dso_device_handle,
														ep,
														data,
														length,
														&actual_length);

...
static const char const *const reg_values[2] = { "VendorID", "ProductID" };
...
NTSTATUS dso_init_device_path(void *dso_device_handle, WCHAR *device_path, const char *prefix);
NTSTATUS dso_ioctl_replay(void *dso_device_handle, IRP *irp);
NTSTATUS dso_ioctl_request(void *dso_device_handle, IRP *irp);
NTSTATUS dso_ioctl_control(void *dso_device_handle, IRP *irp);

// This call-back will have been invoked for each
// known device present or attached to PC
int dso_connect_cb(void *dso_device_handle) {
...
int dso_disconnect_cb(void *dso_device_handle) {
	WCHAR device_dos_pathW[DSO_MAX_PATH_SIZE];
	UNICODE_STRING pathW, dos_pathW;
	DEVICE_OBJECT *device;
	NTSTATUS status;
...
static NTSTATUS WINAPI dso_shutdown(DEVICE_OBJECT *device, IRP *irp) {
	WINE_TRACE("Shutdown Hantek DSO Driver\n");
	libusbdso_stop();
	return STATUS_SUCCESS;
}

NTSTATUS dso_ioctl_replay(void *dso_device_handle, IRP *irp) { // 0x22204E  =>  METHOD_OUT_DIRECT
	NTSTATUS result;
...
NTSTATUS dso_ioctl_request(void *dso_device_handle, IRP *irp) { // 0x222051  =>  METHOD_IN_DIRECT

	IO_STACK_LOCATION *irpsp = IoGetCurrentIrpStackLocation(irp);
	ULONG out_size = irpsp->Parameters.DeviceIoControl.OutputBufferLength;
	PBYTE out_data = (PBYTE) irp->MdlAddress->StartVa
			+ irp->MdlAddress->ByteOffset;

	return libusbdso_write_device(dso_device_handle, out_data, out_size);
}

NTSTATUS dso_ioctl_control(void *dso_device_handle, IRP *irp) { // 0x222059  =>  METHOD_IN_DIRECT
	NTSTATUS result;
	...
	if (result) {
		WINE_ERR_(hantekdsodev)("Hantek DSO (handle: 0x%lx) ioctl_control failed: command %x, value %x\n",
									(long) libusbdso_get_win_device_handle(dso_device_handle),
									ioctl_data->command,
									ioctl_data->value);
	}
...
int fill_wstring(WCHAR *wstring, const char *string) {
	char ch;
	int i;
		...
		for (index = 0;
			...
			if (known_device) {
				...
				if (known_device->name) {
					...
					for (val_index = 0; val_index < REG_VAL_COUNT;
							val_index++) {
						cchName = DSO_REG_KEY_LENGTH;
						status = RegGetValueA(hKey, known_device->name,
								reg_values[val_index],
								RRF_RT_DWORD, &dwType, &valueData, &cchName);
						if (status) {
